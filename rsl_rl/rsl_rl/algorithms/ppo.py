...
def concatenate_given_diagonal_matrices(single_obs_matrix):
...
class PPO:
    ...
    def __init__(
        ...
        if self.sym_loss:
            ...
            for term in obs_terms:
                ...
                matrix_obs.append(concatenate_given_diagonal_matrices(single_obs_matrix))
            self.obs_perm_mat = torch.zeros((stacked_num, stacked_num), device=self.device)
            self.obs_perm_mat = concatenate_given_diagonal_matrices(matrix_obs)
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            if self.sym_loss:
                ...
                mirror_obs = torch.matmul(obs_tensor,self.obs_perm_mat)
                mirror_act = self.policy.actor(mirror_obs)
                m_mirror_act = torch.matmul(mirror_act,self.act_perm_mat)
                sym_loss = (mu_batch-m_mirror_act).pow(2).mean()

            # KL
            if self.desired_kl is not None and self.schedule == "adaptive":
                self.adjust_learning_rate(mu_batch, sigma_batch, old_mu_batch, old_sigma_batch)
    ...
    def adjust_learning_rate(self, mu, sigma, old_mu, old_sigma):
        with torch.inference_mode():
            # kl divergence is the distribution difference between the old and new policies
            kl = torch.sum(
