...
from GRX_humanoid.tasks.locomotion.velocity.mdp.symmetry.gr3 import SymmetryGR3, compute_symmetric_states

@configclass
class PPV233_Mask_WBC(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 30001
    save_interval = 500
    experiment_name = "ppv233_mask_wbc_full"
    ...
    algorithm = MYRslRlPpoAlgorithmCfg(
        ...
        max_grad_norm=1.0,
        sym_loss = True,
        obs_terms = [   {'name': 'base_ang_vel', 'permutation': [-0.0001, 1, -2]},
            ...
            {'name': 'command_enable', 'permutation': [1, 0.0001]},                                     
...
class PPV233_Mask_WBC_Play(PPV233_Mask_WBC):
    #load_run = "2026-01-07_18-43-44"
    #load_run = ".*"
    # load_checkpoint = "model_15000.pt"
    #load_checkpoint = "model_.*.pt"
    pass
@configclass
class PPV233HumanoidRoughPPORunnerCfg_WBC_LOWER(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 10001
    save_interval = 500
    experiment_name = "ppv233_humanoid_rough_wbc_lower"
    empirical_normalization = False
    runner_class = "OnPolicyRunner"
    obs_groups = {
        "policy": ["policy"],
...
@configclass
class PPV233HumanoidRoughPPORunnerCfg_WBC_FULL(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 30001
    save_interval = 500
    experiment_name = "ppv233_humanoid_rough_wbc_full"
    empirical_normalization = False
    runner_class = "OnPolicyRunner"
    obs_groups = {
        "policy": ["policy"],
