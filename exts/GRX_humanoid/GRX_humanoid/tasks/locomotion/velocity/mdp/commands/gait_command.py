...
class GaitCommand(CommandTerm):
    ...
    def _resample_command(self, env_ids):
        ...
        # -- frequency
        if isinstance(self.cfg.ranges.frequencies, tuple):
            self.gait_command[env_ids, 0] = r.uniform_(*self.cfg.ranges.frequencies)
        else:
            frequencies = torch.tensor(self.cfg.ranges.frequencies, device=self.device)
            probabilities = torch.tensor(self.cfg.ranges.frequencies_probs, device=self.device)
            indices = torch.multinomial(probabilities, len(env_ids), replacement=True)
            self.gait_command[env_ids, 0] = frequencies[indices]
        # -- phase offset
