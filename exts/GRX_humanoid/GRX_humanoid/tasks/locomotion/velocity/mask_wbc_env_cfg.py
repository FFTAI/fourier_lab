...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-1.3, 1.3), lin_vel_y=(-0.6, 0.6), ang_vel_z=(-0.8, 0.8), heading=(-3.14, 3.14)
        ),
    )
    ...
    joint_pos_cmd = mdp.UniformJointPosCommandCfg(
        resampling_time_range=(5.0, 10.0),
        debug_vis=False,
        asset_cfg=SceneEntityCfg("robot", joint_names=".*",preserve_order=True),
        ranges_scaled=(0.0, 0.0),#(-1.0, 1.0) max range
    )
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        joint_pos_cmd = ObsTerm(func=mdp.generated_commands, params={"command_name": "joint_pos_cmd"},clip=(-100.0, 100.0))
        # 和指令采样有关，思考下怎么写
        command_enable = ObsTerm(func=mdp.command_enable, params={"command_name": "joint_pos_cmd"})
    ...
    class CriticCfg(ObsGroup):
        ...
        actions = ObsTerm(func=mdp.last_action,clip=(-100.0, 100.0))
        contact_mask = ObsTerm(func=mdp.is_feet_contact, params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*")})
...
class EventCfg:
    ...
    scale_all_rigid_body_com = EventTerm(
        ...
        mode="startup",
        params={"com_range": {"x": (-0.03, 0.03),"y": (-0.03, 0.03),"z": (-0.03, 0.03),},
                "asset_cfg": SceneEntityCfg("robot", body_names=[".*"]),},
    )
    ...
    push_robot = EventTerm(
        ...
        params={"velocity_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5), "z": (-0.1, 0.1), "roll": (-0.2, 0.2), "pitch": (-0.3, 0.3), "yaw": (-0.5, 0.5),}},
    )
...
class RewardsCfg:
    ...
    base_orientation_exp = RewTerm(func=mdp.base_orientation_exp_roll_balance, weight=0.5, params={"std": 0.4, "asset_cfg": SceneEntityCfg("robot")})
    torso_orientation_exp = RewTerm(func=mdp.flat_link_rpy_exp, weight=1.0, params={"std": 0.4, "asset_cfg": SceneEntityCfg("robot", body_names="torso_link")})
    upper_joint_stable = None
    upper_joint_total_track = None
    upper_joint_track = None
    torso_joint_pos = None
    # torso_joint_vel = None
    waist_roll_track_exp = RewTerm(func=mdp.waist_roll_track_exp, weight=1.0, params={"std": 0.3, "asset_cfg": SceneEntityCfg("robot", joint_names="waist_roll_joint")})
    #contact reward
    fly = RewTerm(func=mdp.fly, weight=-0.2, params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*foot_roll.*"), "threshold": 30.0})
    feet_contact_num = RewTerm(func=mdp.FeetContactNumReward, weight=0.5, params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*foot_roll.*"), "threshold": 30.0, "history_length":10, "command_name": "base_velocity",})
    feet_air_time = RewTerm(func=mdp.feet_air_time_positive_biped,weight=2.0,params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*foot_roll.*"),"command_name": "base_velocity","threshold": 0.5,},)
    ...
    foot_alignment = RewTerm(func=mdp.foot_yaw_alignment_reward, weight=0.5,params={"asset_cfg": SceneEntityCfg("robot", body_names=".*foot_roll.*"),"max_angle": 60.0 })
    ...
    feet_too_near = RewTerm(func=mdp.feet_too_near_humanoid, weight=-2.0, params={"asset_cfg": SceneEntityCfg("robot", body_names=[".*foot_roll.*"]), "threshold": 0.14})
    ...
    ang_vel_xy_world_l2 = RewTerm(func=mdp.ang_vel_xy_world_l2, weight=-0.1)
    torso_vel_xy_world_l2 = RewTerm(func=mdp.body_ang_vel_xy_world_l2, weight=-0.2, params={"asset_cfg": SceneEntityCfg("robot", body_names="torso_link")})
    ...
    center_of_mass = RewTerm(func=mdp.center_of_mass, weight=0.5, params={"std": 0.1, "asset_cfg": SceneEntityCfg("robot", body_names=[".*foot_roll.*"])})
    ...
    arm_deviation = RewTerm(func=mdp.arm_deviation_with_cmd_mask, weight=-0.3)
    hand_distance = RewTerm(func=mdp.hand_too_near_to_base, weight=-1.0)
    # centroidal momentum optimization 全身角动量尽量小,手与脚角动量相反且总和尽量小
    # robot_centroidal_momentum = RewTerm(func=mdp.robot_momentum_reward, weight=-0.0, params={"asset_cfg": SceneEntityCfg("robot")})
    robot_limbs_centroidal_momentum_1 = RewTerm(func=mdp.robot_limbs_momentum_reward, weight=-0.0, params={"asset_cfg": SceneEntityCfg("robot")})
    robot_limbs_centroidal_momentum_2 = RewTerm(func=mdp.robot_limbs_momentum_reward, weight=-0.0, params={"asset_cfg": SceneEntityCfg("robot")})
@configclass
...
class CurriculumCfg:
    ...
    task_stage = CurrTerm(
        func=mdp.mask_command_stage,
        # params={     # 之后把curriculum的参数放到这里
        #     "asset_cfg": SceneEntityCfg("robot"),
        #     "num_envs": 4096,
        #     "stage": 1,
        #     "num_steps": 24000,  # 每个阶段的步数
        # },
    )
    ...
    #         "mod_range": {"lin_vel_x": (-1.5, 1.5), "lin_vel_y": (-0.8, 0.8), "ang_vel_z": (-0.6, 0.6)},
    #         "num_steps": 10000,
    #     },
    # )
...
@configclass
class MaskLocomotionEnvCfg(ManagerBasedRLEnvCfg):
